        ...
        @foreach($languages as $lang_key => $lang_name)
            <div class="row js_lang lng_{{ $lang_key }}{{ $main_lang == $lang_key ? ' active_lang' : '' }}">
                ...
                <div class="form-element col-sm-10">
                    <input type="text" class="form-control" name="{{ $key }}_{{ $lang_key }}" value="{{ old($key.'_'.$lang_key) ? old($key.'_'.$lang_key) : (isset($item) ? $item->localize($lang_key, $key) : '') }}" placeholder="{{ $lang_name }}"{{ !empty($required) && $main_lang == $lang_key ? ' required' : '' }} />
        ...
        <div class="row">
            ...
            <div class="form-element col-sm-10">
                <input type="text" class="form-control" name="{{ $key }}{{ isset($locale) ? '_'.$locale : '' }}"
                       value="{{ old($key) ? old($key) : (isset($locale) ? (isset($item) ? $item->localize($locale, $key) : '') : (isset($item) && isset($item->$key) ? $item->$key : '')) }}"
