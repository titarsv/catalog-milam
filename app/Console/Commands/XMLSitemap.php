...
class XMLSitemap extends Command
{
    ...
    public function handle()
    {
        ...
        $this->addLink($site_url.'/ru', null, date("c"), "weekly", "1");
        $this->addLink($site_url, null, date("c"), "weekly", "1", 'ua');
        $this->addLink($site_url.'/en', null, date("c"), "weekly", "1", 'en');
        ...
        foreach (Product::where('visible', 1)->with(['seo', 'image'])->get() as $result) {
            ...
            $this->addLink($site_url.'/ru'.$result->seo->url, !empty($result->image) ? $result->image->url() : null, $date, "weekly", "0.8");
            $this->addLink($site_url.$result->seo->url, !empty($result->image) ? $result->image->url() : null, $date, "weekly", "0.8", 'ua');
            $this->addLink($site_url.'/en'.$result->seo->url, !empty($result->image) ? $result->image->url() : null, $date, "weekly", "0.8", 'en');
        }
        ...
        foreach(Category::where('status', 1)->with('seo', 'image', 'attributes.values')->get() as $result){
            ...
            if(!empty($count)){
                $this->addLink($site_url.'/ru'.$result->seo->url, !empty($result->image) ? $result->image->url() : null, $result->created_at->format("Y-m-d\TH:i:sP"), "daily", "0.8");
                $this->addLink($site_url.$result->seo->url, !empty($result->image) ? $result->image->url() : null, $result->created_at->format("Y-m-d\TH:i:sP"), "daily", "0.8", 'ua');
                ...
                foreach($result->attributes()->where('attributes.id', '!=', 3)->where('attributes.id', '!=', 4)->get() as $attribute){
                    ...
                    foreach($attribute->values as $value){
                        ...
                        $count = Redis::bitcount('count');
                        if(!empty($count)){
                            $this->addLink($site_url.'/ru'.$result->seo->url.'/'.$attr_slug.'-'.$value_slug, !empty($result->image) ? $result->image->url() : null, $result->created_at->format("Y-m-d\TH:i:sP"), "daily", "0.8");
                            $this->addLink($site_url.$result->seo->url.'/'.$attr_slug.'-'.$value_slug, !empty($result->image) ? $result->image->url() : null, $result->created_at->format("Y-m-d\TH:i:sP"), "daily", "0.8", 'ua');
        ...
        foreach($links as $link){
            if($link->url != '/'){
                $this->addLink($site_url.'/ru'.$link->url, null, $link->updated_at->format("Y-m-d\TH:i:sP"), "weekly", "0.4");
                $this->addLink($site_url.$link->url, null, $link->updated_at->format("Y-m-d\TH:i:sP"), "weekly", "0.4", 'ua');
                $this->addLink($site_url.'/en'.$link->url, null, $link->updated_at->format("Y-m-d\TH:i:sP"), "weekly", "0.4", 'en');
            }
        }
