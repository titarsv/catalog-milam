...
class CategoriesController extends Controller
{
	...
	public function showAction($data){
        ...
        if($data->seo->url != $category->seo->url){
            $data->params = explode('/', str_replace((app()->getLocale() == 'ua' ? '' : '/'.app()->getLocale()).$category->seo->url.'/', '', '/'.$data->request->path()));
        }
...
//            }
            ...
            if((count($attributes) == 1 && ((isset($attributes[1]) && count($attributes[1]) == 1) || (isset($attributes[4]) && count($attributes[4]) == 1)))
                || (count($attributes) == 2 && isset($attributes[1]) && isset($attributes[4])) && count($attributes[1]) == 1 && count($attributes[4]) == 1){
                foreach(array_reverse($attributes) as $attr_id => $values){
                    foreach(AttributeValue::with('localization')->whereIn('id', $values)->get() as $value){
                        $name .= ' '.$value->name;
                        $index = true; // индексируем шаблоны
                    }
                }
            }
        ...
        if($data->seo->url == (app()->getLocale() === 'ua' ? '/'.$data->request->path() : substr($data->request->path(), 2))){
            $index = true; // индексируем сеозаписи
        }
...
//            $data->seo->robots = 'index, follow';
		...
		return view('public.catalog')
            ...
            ->with('current_categories', collect((array)$category->get_parent_categories())->pluck('id')->toArray())
    ...
    public function adminEditAction(Request $request, $id){
        ...
        return view('admin.categories.edit')
            ...
            ->with('prev', $prev)
            ->with('categories', $all_categories)
            ->with('products', $category->products()->withPivot('order')->paginate(100))
            ->with('languages', Config::get('app.locales_names'))
	        ->with('editors', localizationFields(['seo_description']))
    ...
    public function adminUpdateProductOrderAction(Request $request){
        ...
        Redis::command('zadd', ['product_category_order_'.$request->category_id, $request->order, $request->product_id]);
