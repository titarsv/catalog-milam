<script>
var ds            = '/';
var home_dir      = ds + "<?php echo e((Config::get('lfm.allow_multi_user')) ? Auth::user()->user_field : ''); ?>";
var shared_folder = ds + "<?php echo e(Config::get('lfm.shared_folder_name')); ?>";
var image_url     = "<?php echo e(asset(Config::get('lfm.images_url'))); ?>";
var file_url      = "<?php echo e(asset(Config::get('lfm.files_url'))); ?>";

$(document).ready(function () {
  bootbox.setDefaults({locale:"<?php echo e(Lang::get('laravel-filemanager::lfm.locale-bootbox')); ?>"});
  // load folders
  loadFolders();
  loadItems();
  setOpenFolders();
...
$('#add-folder').click(function () {
  bootbox.prompt("<?php echo e(Lang::get('laravel-filemanager::lfm.message-name')); ?>", function (result) {
    if (result !== null) {
      createFolder(result);
    }
  });
...
$('#upload-btn').click(function () {
  ...
  function showRequest(formData, jqForm, options) {
    $('#upload-btn').html('<i class="fa fa-refresh fa-spin"></i> <?php echo e(Lang::get("laravel-filemanager::lfm.btn-uploading")); ?>');
    return true;
  }

  function showResponse(responseText, statusText, xhr, $form)  {
    $('#uploadModal').modal('hide');
    $('#upload-btn').html('<?php echo e(Lang::get("laravel-filemanager::lfm.btn-upload")); ?>');
    if (responseText != 'OK'){
      notify(responseText);
    }
    $('#upload').val('');
  ...
  function showError(jqXHR, textStatus, errorThrown) {
    $('#upload-btn').html('<?php echo e(Lang::get("laravel-filemanager::lfm.btn-upload")); ?>');
    if (jqXHR.status == 413) {
      notify('<?php echo e(Lang::get("laravel-filemanager::lfm.error-too-large")); ?>');
    } else if (textStatus == 'error') {
      notify('<?php echo e(Lang::get("laravel-filemanager::lfm.error-other")); ?>' + errorThrown);
    } else {
      notify('<?php echo e(Lang::get("laravel-filemanager::lfm.error-other")); ?>' + textStatus + '<br>' + errorThrown);
    }
  }
...
function loadFolders() {
  $.ajax({
    type: 'GET',
    dataType: 'html',
    url: '<?php echo e(route("unisharp.lfm.getFolders")); ?>',
    data: {
...
function loadItems() {
  ...
  $.ajax({
    type: 'GET',
    dataType: 'html',
    url: '<?php echo e(route("unisharp.lfm.getItems")); ?>',
    data: {
...
function createFolder(folder_name) {
  $.ajax({
    type: 'GET',
    dataType: 'text',
    url: '<?php echo e(route("unisharp.lfm.getAddfolder")); ?>',
    data: {
      name: folder_name,
      working_dir: $('#working_dir').val(),
      type: $('#type').val()
...
function rename(item_name) {
  bootbox.prompt({
    title: "<?php echo e(Lang::get('laravel-filemanager::lfm.message-rename')); ?>",
    value: item_name,
    callback: function (result) {
      if (result !== null) {
        $.ajax({
          type: 'GET',
          dataType: 'text',
          url: '<?php echo e(route("unisharp.lfm.getRename")); ?>',
          data: {
            file: item_name,
            working_dir: $('#working_dir').val(),
            new_name: result,
...
function trash(item_name) {
  bootbox.confirm("<?php echo e(Lang::get('laravel-filemanager::lfm.message-delete')); ?>", function (result) {
    if (result == true) {
      $.ajax({
        type: 'GET',
        dataType: 'text',
        url: '<?php echo e(route("unisharp.lfm.getDelete")); ?>',
        data: {
          working_dir: $('#working_dir').val(),
          items: item_name,
          type: $('#type').val()
...
function cropImage(image_name) {
  $.ajax({
    type: 'GET',
    dataType: 'text',
    url: '<?php echo e(route("unisharp.lfm.getCrop")); ?>',
    data: {
      img: image_name,
      working_dir: $('#working_dir').val(),
      type: $('#type').val()
...
function resizeImage(image_name) {
  $.ajax({
    type: 'GET',
    dataType: 'text',
    url: '<?php echo e(route("unisharp.lfm.getResize")); ?>',
    data: {
      img: image_name,
      working_dir: $('#working_dir').val(),
      type: $('#type').val()
...
function download(file_name) {
  location.href = '<?php echo e(route("unisharp.lfm.getDownload")); ?>?'
  + 'working_dir='
  + $('#working_dir').val()
  + '&type='
  + $('#type').val()
